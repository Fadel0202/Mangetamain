...
def generate_matrix():
    ...
    relevant_ingredients = top_ingredients[
        (top_ingredients > 100) & (top_ingredients < 5000)
    ]
    print(relevant_ingredients)

    relevant_ingredients_exploded = ingredients_exploded[
        ingredients_exploded["ingredients"].str.lower().isin(relevant_ingredients.index)
    ]
    relevant_ingredients_exploded = relevant_ingredients_exploded[["id", "ingredients"]]
    print(relevant_ingredients_exploded.head())

    bin_df = relevant_ingredients_exploded.assign(val=1).pivot_table(
        index="id", columns="ingredients", values="val", fill_value=0
    )
